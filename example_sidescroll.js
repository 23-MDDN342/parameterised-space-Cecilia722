function draw_one_frame(cur_frac) {
  ...
  if (debugView) {
    ...
    for(let i=0; i<arrayOfColours.length-1; i++) {
      ...
      cur_x_pos_reverce = map(cur_frac, 1, 0,grid_points1[i], width*2)
      rect(cur_x_pos - i*width/10, b1_y,b1_size*b1_fac, b1_size); 
      rect(cur_x_pos_reverce - i*width/10, b1_y,b1_size*b1_fac, b1_size); 
      rect(cur_x_pos - i*width/10-width/20, b1_y-silk_space,b1_size*b1_fac, b1_size);
      rect(cur_x_pos_reverce - i*width/10-width/20, b1_y-silk_space,b1_size*b1_fac, b1_size);
  ...
  for(let i=0; i<arrayOfColours.length-1; i++) {
    ...
    let cur_x_pos = map(cur_frac, 0, 1,grid_points1[i]-width/4, width*2)
        cur_x_pos_reverce = map(cur_frac, 1, 0,grid_points1[i]-width/4, width*2)
    
    
      rect(cur_x_pos - i*width/10, b1_y*ii,b1_size*b1_fac, b1_size);
      rect(cur_x_pos_reverce - i*width/10, b1_y*ii,b1_size*b1_fac, b1_size);

      rect(cur_x_pos - i*width/10-width/20, b1_y*ii-silk_space,b1_size*b1_fac, b1_size);
      rect(cur_x_pos_reverce - i*width/10-width/20, b1_y*ii-silk_space,b1_size*b1_fac, b1_size);

      rect(cur_x_pos - i*width/10-width/20, b1_y*ii+silk_space,b1_size*b1_fac, b1_size);
      rect(cur_x_pos_reverce  - i*width/10-width/20, b1_y*ii+silk_space,b1_size*b1_fac, b1_size);

      rect(cur_x_pos - i*width/10+width/20, b1_y*ii+silk_space*2,b1_size*b1_fac, b1_size);
      rect(cur_x_pos_reverce  - i*width/10+width/20, b1_y*ii+silk_space*2,b1_size*b1_fac, b1_size);
